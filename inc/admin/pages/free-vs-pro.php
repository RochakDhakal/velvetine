 ...
 *
 * @example
 * velvetine_get_feature_status_icon( true );  // Outputs a check mark icon.
 * velvetine_get_feature_status_icon( false ); // Outputs a cross icon.
 * velvetine_get_feature_status_icon( 'Coming Soon' ); // Outputs "Coming Soon" as a text status.
 */
function velvetine_get_feature_status_icon( $status ) {
	$allowed_tags = array(
		'div'  => array(
			'class' => true,
		),
...
<div class="velvetine-page__free-pro">
	<h3><?php esc_html_e( 'Go with Cozy Blocks PRO', 'velvetine' ); ?></h3>

	<div class="checkout-button-container top">
		<a href="<?php echo esc_url( 'https://cozythemes.com/pricing-and-plans/' ); ?>" class="checkout-button"><?php esc_html_e( 'Already enjoying our product?', 'velvetine' ); ?></a>
	</div>

	<table>
		<tr>
			<th><?php esc_html_e( 'Feature', 'velvetine' ); ?></th>
			<th><?php esc_html_e( 'Free', 'velvetine' ); ?></th>
			<th><?php esc_html_e( 'Pro', 'velvetine' ); ?></th>
		</tr>

		<?php
		if ( isset( $velvetine_features ) && ! empty( $velvetine_features ) ) :
			foreach ( $velvetine_features as $feature ) {
				?>
				<tr>
					<td>
						<?php
					...
					<td>
						<?php velvetine_get_feature_status_icon( $feature['free'] ); ?>
					</td>

					<td>
						<?php velvetine_get_feature_status_icon( $feature['pro'] ); ?>
					</td>
				</tr>
				<?php
			}
	...
	<br />
	<p><?php esc_html_e( 'Explore More features and details comparison on ', 'velvetine' ); ?>
		<a href="<?php echo esc_url( 'https://cozythemes.com/velvetine/' ); ?>" target="_blank"><?php esc_html_e( 'Here', 'velvetine' ); ?></a>
	</p>
	...
	<div class="refund-policy">
		...
		<div class="policy">
			<h3><?php esc_html_e( 'No Risk, 100% Money Back Guarantee!', 'velvetine' ); ?></h3>
			<p><?php esc_html_e( 'Our Refund Policy secures customer satisfaction and protection. If you find our purchased products unusable, despite our team’s support, you can claim a full refund within 30 days.', 'velvetine' ); ?></p>
		</div>
	</div>

	<!-- Checkout Button Section -->
	<div class="checkout-button-container bottom">
		<?php esc_html_e( 'Ready to take the next step? Let’s get you to', 'velvetine' ); ?> <a href="<?php echo esc_url( 'https://cozythemes.com/pricing-and-plans/' ); ?>" class="checkout-button"><?php esc_html_e( 'Checkout!', 'velvetine' ); ?></a> <?php esc_html_e( '🚀', 'velvetine' ); ?>
	</div>
</div>
<?php
